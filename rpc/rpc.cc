...
void
rpcs::add_reply(unsigned int clt_nonce, unsigned int xid, char *b,
		int sz)
{
	ScopedLock rwl(&reply_window_m_);
	std::map<unsigned int, std::list<reply_t> >::iterator clt;
	...
	std::list<reply_t> *reply_list = &(clt->second);
	assert(reply_list != NULL);

	for (i = reply_list->begin(); i != reply_list->end(); ++i)
	{
		if (i->xid == xid)
		{
			i->buf = b;
...
rpcs::rpcstate_t
rpcs::checkduplicate_and_update(unsigned int clt_nonce, unsigned int xid, unsigned int xid_rep,
        char **b, int *sz)
{
    ...
    assert(reply_list != NULL);
    i = reply_list->begin();
    while (i != reply_list->end()) {
        if (i->xid <= xid_rep) {
            free(i->buf);
            i = reply_list->erase(i);
        } else {
            i++;
        }
    }
